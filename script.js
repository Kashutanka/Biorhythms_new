...
function drawGraph() {
    ...
    const rect = canvas.getBoundingClientRect();
    if (rect.width === 0 || rect.height === 0) return; // 💥 защита от 0
    ...
    // 🔧 Умный ресет prevData, если структура изменилась
    const needsReset =
        prevData.length !== newData.length ||
        prevData.some((p, i) =>
            !p || !newData[i] || p.data.length !== newData[i].data.length
        );

    if (needsReset) {
        prevData = newData.map(p => ({
            person: p.person,
            biorhythms: p.biorhythms,
            data: p.data.map(arr => arr.slice()) // безопасная копия
        }));
    }
    ...
    function animate() {
        ...
        newData.forEach((personObj, personIdx) => {
            ...
            biorhythms.forEach((biorhythm, bioIdx) => {
                ...
                for (let x = 0; x <= width; x++) {
                    const prevY = prevData[personIdx]?.data?.[bioIdx]?.[x] ?? personObj.data[bioIdx][x];
                    const targetY = personObj.data[bioIdx][x];
                    const currentY = prevY + (targetY - prevY) * (currentFrame / maxFrames);
...
// 🛠 Починка при ресайзе — сбрасываем данные
...
function generateRecommendations() {
    ...
    selectedPeople.forEach(person => {
        ...
        selectedBiorhythms.forEach(biorhythm => {
            ...
            let supportNote = "";
            if (percent < -70) {
                const supporters = selectedPeople.filter(p => {
                    if (p.id === person.id) return false;
                    const d = calculateDays(p.birthday, targetDate);
                    const v = calculateBiorhythm(d, biorhythm.cycle);
                    return v > 0.7;
                });

                if (supporters.length > 0) {
                    const names = supporters.map(p => p.name).join(", ");
                    supportNote = `💡 Обратись к ${names} — у ${supporters.length > 1 ? "них" : "неё"} высокий ${biorhythm.name.toLowerCase()} уровень, ${supporters.length > 1 ? "они могут поддержать" : "она может поддержать"}.`;
                }
            }

            // 🧠 Рекомендации по типу биоритма
            if (biorhythm.id === 'physical') {
                if (percent > 70) {
            ...
            personRecommendations.push({
                biorhythm: biorhythm.name,
                percent: percent,
                recommendation: recommendation + (supportNote ? "\n\n" + supportNote : ""),
                emoji: emoji
            });
        });
